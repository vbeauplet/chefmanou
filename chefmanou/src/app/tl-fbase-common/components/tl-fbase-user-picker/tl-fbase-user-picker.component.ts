...
import { TlFbaseUserStaticService } from '../../services/tl-fbase-user.static.service';
import * as firebase from 'firebase';

/**
 * User picker is a UI/behavior component that enables to select user among a list of users registered within the firebase database
 *
 * @author vbeauplet
 */
@Component({
  selector: 'tl-fbase-user-picker',
  host: { 
    '[class]' : 'this.size + " margined row-dir flex-block"',
    },
  templateUrl: './tl-fbase-user-picker.component.html',
  styleUrls: ['./tl-fbase-user-picker.component.scss']
})
export class TlFbaseUserPickerComponent implements OnInit {
   ...
   */
  @Input() initialSelectedUsers: TlUser[] = [];
  ...
  constructor(
      private userService: TlFbaseUserStaticService) { }
  ...
  public onClickUser(user: TlUser){
   ...
   */
  private switchUserStatus(user: TlUser){
   ...
   */
  public addUserOnServer(user: TlUser) {
   ...
   * Promise is resolved if update of the recipe went well, rejected if not
   */
  public removeUserOnServer(user: TlUser) {
    // Create property update
    let sdbPropertyUpdate = {};
